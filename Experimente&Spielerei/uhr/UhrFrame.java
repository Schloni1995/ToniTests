...
import rtModuls.de.rtObjects.gui.RTFrame;
import rtModuls.de.rtObjects.rtFunction.RTScreenFunctions;

public class UhrFrame extends RTFrame
{
	private static final long serialVersionUID = 1L;

	public static void main(final String[] args)
	{
		System.out.println(RTColors.getRimeOrange());
		new UhrFrame();
	}
	...
	public UhrFrame()
	{
		final ImageIcon icon = new ImageIcon("M:/workspace/TonisTests/Grundlagen/ZeitSprache/icon256.png");
		setTitle("Uhrzeit");
		getContentPane().setBackground(Color.gray);
		setIconImage(icon.getImage());
		setDefaultCloseOperation(JFrame.EXIT_ON_CLOSE);
		setPreferredSize(new Dimension(500, 250));
		final RTScreenFunctions rtsf = new RTScreenFunctions();
		// rtsf.setProgOn2ndScreen(this);
		// rtsf.setFullDisplaySize(this);
		...
		final Timer timer = new Timer(1000, e ->
		{
			...
			if (dayNameString.equals("Montag") ^ dayNameString.equals("Mittwoch") ^ dayNameString.equals("Freitag"))
			{
				...
				else
				{
					...
					uhrZeit.setForeground(Color.BLACK);
				}
			}
			...
			System.out.println("Frei=" + frei + " Frühstückspause=" + pause1 + " Mittagspause=" + pause2);
		});
		timer.start();
		add(uhrZeit, BorderLayout.CENTER);
		add(daTum, BorderLayout.SOUTH);
		pack();
		rtsf.setCenter(UhrFrame.this);
